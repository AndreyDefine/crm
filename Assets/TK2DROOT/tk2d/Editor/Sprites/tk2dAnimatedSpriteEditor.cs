...
class tk2dAnimatedSpriteEditor : tk2dSpriteEditor {
    ...
    struct AnimationName {
        ...
        public AnimationName(int index, string name) {
            this.index = index;
            this.name = name;
        }
    }

    class AnimationNameComparer : IComparer {
        int IComparer.Compare(object l, object r) {
            return ((AnimationName)l).name.CompareTo(((AnimationName)r).name);
        }
    }

    static string[] sortedSpriteAnimationNames = new string[0];
    static int[] sortedIndexSpriteAnimationNames = new int[0];
    ...
    void Init() {
        if (!initialized) {
            ...
            if (animLibs != null) {
                ...
                AnimationName[] sortedList = new AnimationName[animLibs.Length];
                for (int i = 0; i < animLibs.Length; ++i) {
                    animLibNames[i] = animLibs[i].name;
                    sortedList[i] = new AnimationName(i, animLibs[i].name);
                }
                System.Array.Sort(sortedList, new AnimationNameComparer());

                sortedSpriteAnimationNames = new string[animLibs.Length];
                sortedIndexSpriteAnimationNames = new int[animLibs.Length];

                for (int i=0; i<sortedList.Length; i++) {
                    sortedSpriteAnimationNames[i] = sortedList[i].name;
                    sortedIndexSpriteAnimationNames[i] = sortedList[i].index;
                }
            }
    ...
    public override void OnInspectorGUI() {
        ...
        if (animLibs == null) {
            ...
            int newAnimLib = selAnimLib;
            if (sortedIndexSpriteAnimationNames != null) {
                for (int i=0; i< sortedIndexSpriteAnimationNames.Length; i++) {
                    if (sortedIndexSpriteAnimationNames[i] == selAnimLib) {
                        newAnimLib = EditorGUILayout.Popup("Anim Lib", i, sortedSpriteAnimationNames);
                        newAnimLib = sortedIndexSpriteAnimationNames[newAnimLib];
                        break;
                    }
                }
            } else {
                newAnimLib = EditorGUILayout.Popup("Anim Lib", selAnimLib, animLibNames);
            }
